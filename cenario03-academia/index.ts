...
function realizarCheckin(checkin: ICheckin): IResultadoCheckin {
    ...
    const jaFezCheckin = checkIns.some(
        c => c.alunoId === checkin.alunoId && c.data === data
    )
...
function cancelarPlano(alunoId: number): IResultadoCancelamento {
    ...
    const mesesRestantes = Math.ceil(diffDias / 30)


    const planoCalc = calcularPlano(aluno.plano, aluno.personal)


    const valorRestante = mesesRestantes * planoCalc.valorMensal
